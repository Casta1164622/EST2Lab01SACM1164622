#Obtenido en https://gist.github.com/benben233/2c8a2a8ab44a7beabad0df1b6658232e
#Agregada funcion de encontrar por valor
...
class BPlusTree(object):
    ...
    def searhDPI(self, key):
        leaf = self.find(key)
        for item in leaf.keys:
            if key[0]['dpi'] in item[0]['dpi']:
                return leaf[item]

    def searchNombre(self, key):
        results = []
        leaf = self.find(({"dpi":"0"},{}))
        while leaf.next != None:
            for item in leaf.keys:
                if key[1]['name'] in item[1]['name']:
                    results.append(leaf[item])
            leaf = leaf.next
        return results
